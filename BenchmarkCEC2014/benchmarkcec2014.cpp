/*
  ...
  Janez Brest for Linux, Dec. 21st 2013
*/



#include "benchmarkcec2014.h"
...
int BenchmarkCec2014::getfOpt(int i)
{
    return i*100;
}

BenchmarkCec2014::BenchmarkCec2014(int D)
{
    nx = D;
    ini_flag = 0;
    M=NULL;
...
void BenchmarkCec2014::cec14_test_func(double *x, double *f, int func_num)
{
    int unused __attribute__((unused)); // return code using for scanf and fscanf, Janez
    int cf_num=10,i,j;
    if (ini_flag==1)
    {
    ...
    if (ini_flag==0)
    {
        //std::cout<<"tu sem "<<func_num<<std::endl;
        FILE *fpt;
        char FileName[256];
        free(M);
        free(OShift);
        ...
        if (func_num<23)
        {
            ...
            for (i=0; i<nx*nx; i++)
            {
                unused=fscanf(fpt,"%lf",&M[i]);   // %Lf --> %lf
            }
        }
        else
        {
...
void BenchmarkCec2014::sphere_func (double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag) /* Sphere */
{
    int i;
    f[0] = 0.0;
...
void BenchmarkCec2014::ellips_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Ellipsoidal */
{
    int i;
    f[0] = 0.0;
    sr_func (x, z, nx, Os, Mr,1.0, s_flag, r_flag); /* shift and rotate */
...
void BenchmarkCec2014::bent_cigar_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Bent_Cigar */
{
    int i;
    sr_func (x, z, nx, Os, Mr,1.0, s_flag, r_flag); /* shift and rotate */
...
void BenchmarkCec2014::discus_func (double *x, double *f, int nx, double *Os,double *Mr, int s_flag, int r_flag) /* Discus */
{
    int i;
    sr_func (x, z, nx, Os, Mr,1.0, s_flag, r_flag); /* shift and rotate */
...
void BenchmarkCec2014::dif_powers_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Different Powers */
{
    int i;
    f[0] = 0.0;
    sr_func (x, z, nx, Os, Mr, 1.0, s_flag, r_flag); /* shift and rotate */
...
void BenchmarkCec2014::rosenbrock_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Rosenbrock's */
{
    int i;
    double tmp1,tmp2;
    f[0] = 0.0;
...
void BenchmarkCec2014::schaffer_F7_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Schwefel's 1.2  */
{
    int i;
    double tmp;
    f[0] = 0.0;
...
void BenchmarkCec2014::ackley_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Ackley's  */
{
    int i;
    double sum1, sum2;
    sum1 = 0.0;
...
void BenchmarkCec2014::weierstrass_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Weierstrass's  */
{
    int i,j,k_max;
    double sum,sum2, a, b;
    a = 0.5;
...
sum2=0.0; // Without warning when compiling
    for (i=0; i<nx; i++)
    {
        sum = 0.0;
        sum2 = 0.0;
...
void BenchmarkCec2014::griewank_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Griewank's  */
{
    int i;
    double s, p;
    s = 0.0;
...
void BenchmarkCec2014::rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Rastrigin's  */
{
    int i;
    f[0] = 0.0;
...
void BenchmarkCec2014::step_rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Noncontinuous Rastrigin's  */
{
    int i;
    f[0]=0.0;
    for (i=0; i<nx; i++)
...
void BenchmarkCec2014::schwefel_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Schwefel's  */
{
    int i;
    double tmp;
    f[0]=0.0;
...
void BenchmarkCec2014::katsuura_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Katsuura  */
{
    int i,j;
...
void BenchmarkCec2014::bi_rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Lunacek Bi_rastrigin Function */
{
    int i;
...
void BenchmarkCec2014::grie_rosen_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Griewank-Rosenbrock  */
{
    int i;
...
void BenchmarkCec2014::escaffer6_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Expanded Scaffer??s F6  */
{
    int i;
    double temp1, temp2;
...
void BenchmarkCec2014::happycat_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* HappyCat, provdided by Hans-Georg Beyer (HGB) */
...
void BenchmarkCec2014::hgbat_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* HGBat, provdided by Hans-Georg Beyer (HGB)*/
/* original global optimum: [-1,-1,...,-1] */
{
    int i;
    double alpha,r2,sum_z;
...
void BenchmarkCec2014::hf01 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 1 */
{
    int i,tmp,cf_num=3;
    double fit[3];
    int G[3],G_nx[3];
...
void BenchmarkCec2014::hf02 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 2 */
{
    int i,tmp,cf_num=3;
    double fit[3];
    int G[3],G_nx[3];
...
void BenchmarkCec2014::hf03 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 3 */
{
    int i,tmp,cf_num=4;
    double fit[4];
    int G[4],G_nx[4];
...
void BenchmarkCec2014::hf04 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 4 */
{
    int i,tmp,cf_num=4;
    double fit[4];
    int G[4],G_nx[4];
...
void BenchmarkCec2014::hf05 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 5 */
{
    int i,tmp,cf_num=5;
    double fit[5];
    int G[5],G_nx[5];
...
void BenchmarkCec2014::hf06 (double *x, double *f, int nx, double *Os,double *Mr,int *S,int s_flag,int r_flag) /* Hybrid Function 6 */
{
    int i,tmp,cf_num=5;
    double fit[5];
    int G[5],G_nx[5];
...
void BenchmarkCec2014::cf01 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 1 */
{
    int i,cf_num=5;
    double fit[5];
...
void BenchmarkCec2014::cf02 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 2 */
{
    int i,cf_num=3;
    double fit[3];
...
void BenchmarkCec2014::cf03 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 3 */
{
    int i,cf_num=3;
    double fit[3];
...
void BenchmarkCec2014::cf04 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 4 */
{
    int i,cf_num=5;
    double fit[5];
...
void BenchmarkCec2014::cf05 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 4 */
{
    int i,cf_num=5;
    double fit[5];
...
void BenchmarkCec2014::cf06 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag) /* Composition Function 6 */
{
    int i,cf_num=5;
    double fit[5];
    double delta[5] = {10,20,30,40,50};
...
void BenchmarkCec2014::cf07 (double *x, double *f, int nx, double *Os,double *Mr,int *SS,int r_flag) /* Composition Function 7 */
{
    int i,cf_num=3;
    double fit[3];
    double delta[3] = {10,30,50};
...
void BenchmarkCec2014::cf08 (double *x, double *f, int nx, double *Os,double *Mr,int *SS,int r_flag) /* Composition Function 8 */
{
    int i,cf_num=3;
    double fit[3];
    double delta[3] = {10,30,50};
...
void BenchmarkCec2014::shiftfunc (double *x, double *xshift, int nx,double *Os)
{
    int i;
    for (i=0; i<nx; i++)
    {
...
void BenchmarkCec2014::rotatefunc (double *x, double *xrot, int nx,double *Mr)
{
    int i,j;
    for (i=0; i<nx; i++)
    {
...
void BenchmarkCec2014::sr_func (double *x, double *sr_x, int nx, double *Os,double *Mr, double sh_rate, int s_flag,int r_flag) /* shift and rotate */
{
    int i;
...
void BenchmarkCec2014::asyfunc (double *x, double *xasy, int nx, double beta)
{
    int i;
    for (i=0; i<nx; i++)
    {
...
void BenchmarkCec2014::oszfunc (double *x, double *xosz, int nx)
{
    int i,sx;
    double c1,c2,xx;

xx=0; // without compiling warning
    for (i=0; i<nx; i++)
    {
        if (i==0||i==nx-1)
        {
...
void BenchmarkCec2014::cf_cal(double *x, double *f, int nx, double *Os,double * delta,double * bias,double * fit, int cf_num)
{
    int i,j;
    double *w;
